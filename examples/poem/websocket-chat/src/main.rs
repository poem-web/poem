...
///
/// Returns a html page include simple input & submit button for communicating with backend service
/// through websocket
#[handler]
...
/// sender ([`Data<T>`]) was used for exchanging data through the T (tokio broadcast channel here)
/// ws will register 2 thread, 1 for receive the message sent by other service through websocket, and
/// send into the channel init before; and the other receive the message from internal channel and
...
///     );
/// ```
/// register the websocket api in router, which would take action on handling the messages exchanged
/// through websocket
...
/// 3. send message
/// 4. service will send message back through websocket with log print in console
#[tokio::main]
