...
pub(crate) fn create_linker(engine: &Engine) -> Result<Linker<ExecState>> {
    ...
    linker.func_wrap3_async("env", "request_read_body", request_read_body)?;
    linker.func_wrap1_async("env", "response_status", response_status)?;
    linker.func_wrap2_async("env", "response_header_map", response_header_map)?;
    linker.func_wrap2_async("env", "response_body", response_body)?;
...
fn request_read_body<'a>(
    ...
    Box::new(async move {
        let memory = get_memory(&mut caller)?;
        let (memory, state) = memory.data_and_store_mut(caller.as_context_mut());
        let data = get_memory_slice_mut(memory, buf, buf_len)?;
        match state.request_body.read(data).await {
            Ok(n) => {
                set_ret_len(memory, ret_len, n as u32)?;
                Ok(0)
            }
            Err(_) => Ok(-1),
        }
...
pub(crate) fn response_status<'a>(
    caller: Caller<'a, ExecState>,
    status: u32,
...
pub(crate) fn response_body<'a>(
    ...
    Box::new(async move {
        ...
        let res = state
            .response_sender
            .send(ResponseMsg::Body(data.to_vec()))
            .await;
